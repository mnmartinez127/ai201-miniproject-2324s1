...
if __name__ == "__main__":
    ...
    datasets_name = ["Coconut Tree Disease Dataset","Coconut Leaf Dataset for Pest Identification"]
    for data_name in datasets_name:
        ...
        if not use_cache: #regenerate cache if not complete
            ...
            X_train,y_train = train_data.balance_data(X_train,y_train,method='oversample',random_state = random_state) #balance training dataset before using
        ...
        if not os.path.isfile(os.path.join(results_path,"result-accuracy.csv")):
            ...
            for func_name,ufunc in activation_functions.items():
                for num_hidden in range(1000,10001,1000):
        ...
        perf = perf.sort_values(by=["lbp","activation function","color space","hidden nodes"],ignore_index=True)
        perf.to_csv(os.path.join(results_path,"result-accuracy.csv"))
        act_lines = {"Leaky ReLU":'r',"Logistic":'g',"Swish":'b'}
        ...
        ax2.set_title(f"Training time: Color Histogram Features")
        ax2.set_xlim([0,int(perf["hidden nodes"].max())+1]) #establish range of values
        ax2.set_ylim([0,(int(perf["training time"].max()+1)*1.1)])
        ax2.set_xlabel("Number of hidden nodes")
        ax2.set_ylabel("Training time (s)")

        fig3,ax3=plt.subplots()
        ax3.set_title(f"Testing time: Color Histogram Features")
        ax3.set_xlim([0,int(perf["hidden nodes"].max())+1]) #establish range of values
        ax3.set_ylim([0,(int(perf["testing time"].max()+1)*1.1)])
        ax3.set_xlabel("Number of hidden nodes")
        ax3.set_ylabel("Testing time (s)")


        fig4,ax4=plt.subplots()
        ax4.set_title(f"Accuracy: Color Histogram + LBP Features")
        ax4.set_xlim([0,int(perf["hidden nodes"].max())+1]) #establish range of values
        ax4.set_ylim([0,1])
        ax4.set_xlabel("Number of hidden nodes")
        ax4.set_ylabel("Accuracy (s)")

        fig5,ax5=plt.subplots()
        ax5.set_title(f"Training time: Color Histogram + LBP Features")
        ax5.set_xlim([0,int(perf["hidden nodes"].max())+1]) #establish range of values
        ax5.set_ylim([0,(int(perf["training time"].max()+1)*1.1)])
        ax5.set_xlabel("Number of hidden nodes")
        ax5.set_ylabel("Training time (s)")

        fig6,ax6=plt.subplots()
        ax6.set_title(f"Testing time: Color Histogram + LBP Features")
        ax6.set_xlim([0,int(perf["hidden nodes"].max())+1]) #establish range of values
        ax6.set_ylim([0,(int(perf["testing time"].max()+1)*1.1)])
        ax6.set_xlabel("Number of hidden nodes")
        ax6.set_ylabel("Testing time (s)")

        perf_groups = perf.groupby(["activation function","color space","lbp"])
        for name,group in perf_groups:
            print(name)
            print(group)
            if name[2] == False:
                ax1.plot(group["hidden nodes"],group["accuracy"],f'{act_lines[name[0]]}o{color_lines[name[1]]}',label=f"{name[0]}-{name[1]}")
                ax2.plot(group["hidden nodes"],group["training time"],f'{act_lines[name[0]]}o{color_lines[name[1]]}',label=f"{name[0]}-{name[1]}")
                ax3.plot(group["hidden nodes"],group["testing time"],f'{act_lines[name[0]]}o{color_lines[name[1]]}',label=f"{name[0]}-{name[1]}")
            else:
                ax4.plot(group["hidden nodes"],group["accuracy"],f'{act_lines[name[0]]}o{color_lines[name[1]]}',label=f"{name[0]}-{name[1]}")
                ax5.plot(group["hidden nodes"],group["training time"],f'{act_lines[name[0]]}o{color_lines[name[1]]}',label=f"{name[0]}-{name[1]}")
                ax6.plot(group["hidden nodes"],group["testing time"],f'{act_lines[name[0]]}o{color_lines[name[1]]}',label=f"{name[0]}-{name[1]}")
        ...
        fig2.tight_layout()
        fig2.show()
        fig2.savefig(os.path.join(results_path,f"Training Time"),bbox_inches="tight",pad_inches=1.0) #save response curve to file


        ax3.legend(loc="upper center",bbox_to_anchor=(0.5,-0.2),ncols=3) #legend showing name of each curve
        fig3.tight_layout()
        fig3.show()
        fig3.savefig(os.path.join(results_path,f"Testing Time"),bbox_inches="tight",pad_inches=1.0) #save response curve to file


        ax4.legend(loc="upper center",bbox_to_anchor=(0.5,-0.2),ncols=3) #legend showing name of each curve
        fig4.tight_layout()
        fig4.show()
        fig4.savefig(os.path.join(results_path,f"Accuracy_lbp"),bbox_inches="tight",pad_inches=1.0) #save response curve to file


        ax5.legend(loc="upper center",bbox_to_anchor=(0.5,-0.2),ncols=3) #legend showing name of each curve
        fig5.tight_layout()
        fig5.show()
        fig5.savefig(os.path.join(results_path,f"Training Time_lbp"),bbox_inches="tight",pad_inches=1.0) #save response curve to file


        ax6.legend(loc="upper center",bbox_to_anchor=(0.5,-0.2),ncols=3) #legend showing name of each curve
        fig6.tight_layout()
        fig6.show()
        fig6.savefig(os.path.join(results_path,f"Testing Time_lbp"),bbox_inches="tight",pad_inches=1.0) #save response curve to file
